...
/**
 * // TODO_MA 注释： 资源管理器的 YARN 实施。 通过资源框架 YARN 启动系统时使用。
 * The yarn implementation of the resource manager.
 * Used when the system is started via the resource framework YARN.
 */
public class YarnResourceManager extends ActiveResourceManager<YarnWorkerNode> implements AMRMClientAsync.CallbackHandler, NMClientAsync.CallbackHandler {
	...
	public YarnResourceManager(RpcService rpcService, ResourceID resourceId, Configuration flinkConfig, Map<String, String> env,
		HighAvailabilityServices highAvailabilityServices, HeartbeatServices heartbeatServices, SlotManager slotManager,
		ResourceManagerPartitionTrackerFactory clusterPartitionTrackerFactory, JobLeaderIdService jobLeaderIdService,
		ClusterInformation clusterInformation, FatalErrorHandler fatalErrorHandler, @Nullable String webInterfaceUrl,
		ResourceManagerMetricGroup resourceManagerMetricGroup) {
		super(flinkConfig, env, rpcService, resourceId, highAvailabilityServices, heartbeatServices, slotManager, clusterPartitionTrackerFactory,
			jobLeaderIdService, clusterInformation, fatalErrorHandler, resourceManagerMetricGroup);
		this.yarnConfig = new YarnConfiguration();
		this.workerNodeMap = new ConcurrentHashMap<>();
		final int yarnHeartbeatIntervalMS = flinkConfig.getInteger(YarnConfigOptions.HEARTBEAT_DELAY_SECONDS) * 1000;

		final long yarnExpiryIntervalMS = yarnConfig
			.getLong(YarnConfiguration.RM_AM_EXPIRY_INTERVAL_MS, YarnConfiguration.DEFAULT_RM_AM_EXPIRY_INTERVAL_MS);
		...
		this.workerSpecContainerResourceAdapter = new WorkerSpecContainerResourceAdapter(flinkConfig, yarnConfig
			.getInt(YarnConfiguration.RM_SCHEDULER_MINIMUM_ALLOCATION_MB, YarnConfiguration.DEFAULT_RM_SCHEDULER_MINIMUM_ALLOCATION_MB),
			yarnConfig.getInt(YarnConfiguration.RM_SCHEDULER_MINIMUM_ALLOCATION_VCORES,
				YarnConfiguration.DEFAULT_RM_SCHEDULER_MINIMUM_ALLOCATION_VCORES), yarnConfig
			.getInt(YarnConfiguration.RM_SCHEDULER_MAXIMUM_ALLOCATION_MB, YarnConfiguration.DEFAULT_RM_SCHEDULER_MAXIMUM_ALLOCATION_MB),
		...
		this.matchingStrategy = flinkConfig.getBoolean(
			YarnConfigOptionsInternal.MATCH_CONTAINER_VCORES) ? WorkerSpecContainerResourceAdapter.MatchingStrategy.MATCH_VCORE : WorkerSpecContainerResourceAdapter.MatchingStrategy.IGNORE_VCORE;
	}

	protected AMRMClientAsync<AMRMClient.ContainerRequest> createAndStartResourceManagerClient(YarnConfiguration yarnConfiguration,
		int yarnHeartbeatIntervalMillis, @Nullable String webInterfaceUrl) throws Exception {

		/*************************************************
		 * 
		 *  注释： 创建 resourceManagerClient
		 */
		AMRMClientAsync<AMRMClient.ContainerRequest> resourceManagerClient = AMRMClientAsync
			.createAMRMClientAsync(yarnHeartbeatIntervalMillis, this);

		/*************************************************
		 * 
		 *  注释： 初始化 和 启动 resourceManagerClient
		 */
		...
		/*************************************************
		 * 
		 *  注释： 注册 ApplicationMaster
		 */
		final RegisterApplicationMasterResponse registerApplicationMasterResponse = resourceManagerClient
			.registerApplicationMaster(hostPort.f0, restPort, webInterfaceUrl);
	...
	private void updateMatchingStrategy(final RegisterApplicationMasterResponse registerApplicationMasterResponse) {
		final Optional<Set<String>> schedulerResourceTypesOptional = registerApplicationMasterResponseReflector
			.getSchedulerResourceTypeNames(registerApplicationMasterResponse);
		...
		if(schedulerResourceTypesOptional.isPresent()) {
			...
			matchingStrategy = types.contains(
				"CPU") ? WorkerSpecContainerResourceAdapter.MatchingStrategy.MATCH_VCORE : WorkerSpecContainerResourceAdapter.MatchingStrategy.IGNORE_VCORE;
		} else {
	...
	protected void initialize() throws ResourceManagerException {
		try {
			/*************************************************
			 * 
			 *  注释： 启动一个 ResourceManager Client
			 */
			resourceManagerClient = createAndStartResourceManagerClient(yarnConfig, yarnHeartbeatIntervalMillis, webInterfaceUrl);
		} catch(Exception e) {
	...
	public boolean startNewWorker(WorkerResourceSpec workerResourceSpec) {
		...
		/*************************************************
		 * 
		 *  注释： 申请 YARN Container 用于启动一个 TaskExecutor
		 */
	...
	private void onContainersOfResourceAllocated(Resource resource, List<Container> containers) {
		final List<WorkerResourceSpec> pendingWorkerResourceSpecs = workerSpecContainerResourceAdapter.getWorkerSpecs(resource, matchingStrategy)
			.stream().flatMap(spec -> Collections.nCopies(getNumRequestedNotAllocatedWorkersFor(spec), spec).stream())
			.collect(Collectors.toList());

		int numPending = pendingWorkerResourceSpecs.size();
		log.info("Received {} containers with resource {}, {} pending container requests.", containers.size(), resource, numPending);
		...
		final Iterator<AMRMClient.ContainerRequest> pendingRequestsIterator = getPendingRequestsAndCheckConsistency(resource,
		...
		log.info("Accepted {} requested containers, returned {} excess containers, {} pending container requests of resource {}.", numAccepted,
			numExcess, numPending, resource);
	}
	...
	private void startTaskExecutorInContainer(Container container, WorkerResourceSpec workerResourceSpec, ResourceID resourceId) {
		...
		try {
			...
			ContainerLaunchContext taskExecutorLaunchContext = createTaskExecutorLaunchContext(resourceId.toString(),
				container.getNodeId().getHost(), TaskExecutorProcessUtils.processSpecFromWorkerResourceSpec(flinkConfig, workerResourceSpec));
	...
	private Collection<AMRMClient.ContainerRequest> getPendingRequestsAndCheckConsistency(Resource resource, int expectedNum) {
		final Collection<Resource> equivalentResources = workerSpecContainerResourceAdapter
			.getEquivalentContainerResource(resource, matchingStrategy);
		final List<? extends Collection<AMRMClient.ContainerRequest>> matchingRequests = equivalentResources.stream().flatMap(
			equivalentResource -> resourceManagerClient.getMatchingRequests(RM_REQUEST_PRIORITY, ResourceRequest.ANY, equivalentResource)
		...
		Preconditions.checkState(matchingContainerRequests.size() == expectedNum,
			"The RMClient's and YarnResourceManagers internal state about the number of pending container requests for resource %s has diverged. " + "Number client's pending container requests %s != Number RM's pending container requests %s.",
	...
	private void requestYarnContainerIfRequired() {
		for(Map.Entry<WorkerResourceSpec, Integer> requiredWorkersPerResourceSpec : getRequiredResources().entrySet()) {
			...
			while(requiredWorkersPerResourceSpec.getValue() > getNumRequestedNotRegisteredWorkersFor(workerResourceSpec)) {
				...
				Preconditions.checkState(requestContainerSuccess, "Cannot request container for worker resource spec {}.", workerResourceSpec);
			}
		}
	}
	...
	private boolean requestYarnContainer(WorkerResourceSpec workerResourceSpec) {
		...
		if(containerResourceOptional.isPresent()) {
			...
			log.info("Requesting new TaskExecutor container with resource {}. Number pending workers of this resource is {}.",
				workerResourceSpec, numPendingWorkers);
			return true;
		} else {
			return false;
		}
	...
	private ContainerLaunchContext createTaskExecutorLaunchContext(String containerId, String host,
		...
		final ContaineredTaskManagerParameters taskManagerParameters = ContaineredTaskManagerParameters
			.create(flinkConfig, taskExecutorProcessSpec);

		log.info("TaskExecutor {} will be started on {} with {}.", containerId, host, taskExecutorProcessSpec);
		...
		ContainerLaunchContext taskExecutorLaunchContext = Utils
			.createTaskExecutorContext(flinkConfig, yarnConfig, env, taskManagerParameters, taskManagerDynamicProperties, currDir,
