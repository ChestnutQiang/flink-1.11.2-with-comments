...
public class LocalExecutor implements PipelineExecutor {
	...
	public static LocalExecutor createWithFactory(Configuration configuration,
	...
	private JobGraph getJobGraph(Pipeline pipeline, Configuration configuration) throws MalformedURLException {
		...
		// to <num taskmanagers> * <num task slots>. Might be questionable but we keep the behaviour for now.
		if(pipeline instanceof Plan) {
			Plan plan = (Plan) pipeline;
			final int slotsPerTaskManager = configuration.getInteger(TaskManagerOptions.NUM_TASK_SLOTS, plan.getMaximumParallelism());
