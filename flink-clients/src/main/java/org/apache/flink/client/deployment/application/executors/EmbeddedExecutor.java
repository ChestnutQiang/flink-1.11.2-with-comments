...
public class EmbeddedExecutor implements PipelineExecutor {
	...
	private CompletableFuture<JobClient> submitAndGetJobClientFuture(final Pipeline pipeline,
		...
		/*************************************************
		 *
		 *  注释： 通过 StreamGraph 加载得到 JobGraph
		 */
		...
		return jobSubmissionFuture.thenApplyAsync(jobID -> jobClientCreator.getJobClient(actualJobId));
	}

	private static CompletableFuture<JobID> submitJob(final Configuration configuration, final DispatcherGateway dispatcherGateway,
		final JobGraph jobGraph, final Time rpcTimeout) {
