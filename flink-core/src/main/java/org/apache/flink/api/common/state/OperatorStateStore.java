...
/**
 * // TODO_MA 注释： 通过 CheckpointedFunction 接口既可以获取 Operator State，
 * // TODO_MA 注释： 也可以获取 Keyed State，这两类状态分别通过 OperatorStateStore
 * // TODO_MA 注释： 和 KeyedStateStore 这两个接口作为桥梁来进行管理。
 *
 * // TODO_MA 注释： 该接口包含用于向托管存储注册操作员状态的方法。
 * This interface contains methods for registering operator state with a managed store.
 *
 * // TODO_MA 注释： OperatorStateStore 定义了用于创建和管理托管状态的方法，
 * // TODO_MA 注释： 分别对应 ListState，union ListState 以及 BroadcastState 。
 * // TODO_MA 注释： 其中 ListState 和 Union ListState 的底层存储是一致的，只是在状态恢复的时候状态的分配模式不一致。
 */
@PublicEvolving
