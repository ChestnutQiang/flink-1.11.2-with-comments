...
/**
 * // TODO_MA 注释： RuntimeContext 包含有关在其中执行功能的上下文的信息。
 * // TODO_MA 注释： 函数的每个并行实例都将具有一个上下文，通过它可以
 * // TODO_MA 注释： 访问静态上下文信息（例如当前的并行性）以及其他构造（例如累加器和广播变量）。
 ...
 *
 * // TODO_MA 注释： 函数可以在运行时通过调用 {@link AbstractRichFunction＃getRuntimeContext（）} 获得 RuntimeContext。
 * <p>A function can, during runtime, obtain the RuntimeContext via a call to
 * {@link AbstractRichFunction#getRuntimeContext()}.
 *
 * // TODO_MA 注释： 对于 Keyed State，通常都是通过 RuntimeContext 实例来获取，这通常需要在 rich functions 中才可以做到。
 * // TODO_MA 注释： 注意，使用 Keyed State 一定要在 KeyedStream 上进行操作。
 */
@Public
