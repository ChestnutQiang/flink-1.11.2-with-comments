...
public class AsyncWaitOperator<IN, OUT> extends AbstractUdfStreamOperator<OUT, AsyncFunction<IN, OUT>> implements OneInputStreamOperator<IN, OUT>, BoundedOneInput {
	...
	public AsyncWaitOperator(@Nonnull AsyncFunction<IN, OUT> asyncFunction, long timeout, int capacity,
		@Nonnull AsyncDataStream.OutputMode outputMode, @Nonnull ProcessingTimeService processingTimeService,
	...
	public void processElement(StreamRecord<IN> element) throws Exception {
		...
		if(timeout > 0L) {
			...
			final ScheduledFuture<?> timeoutTimer = getProcessingTimeService()
				.registerTimer(timeoutTimestamp, timestamp -> userFunction.timeout(element.getValue(), resultHandler));
	...
	public void snapshotState(StateSnapshotContext context) throws Exception {
		...
		ListState<StreamElement> partitionableState = getOperatorStateBackend()
		...
		try {
			...
			throw new Exception(
				"Could not add stream element queue entries to operator state " + "backend of operator " + getOperatorName() + '.', e);
		}
	}
	...
	private class ResultHandler implements ResultFuture<OUT> {
		...
		private void processInMailbox(Collection<OUT> results) {
			...
			mailboxExecutor.execute(() -> processResults(results), "Result in AsyncWaitOperator of input %s", results);
		}
		...
		public void completeExceptionally(Throwable error) {
			...
			getContainingTask().getEnvironment()
				.failExternally(new Exception("Could not complete the stream element: " + inputRecord + '.', error));
