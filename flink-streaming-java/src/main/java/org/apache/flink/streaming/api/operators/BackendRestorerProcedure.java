...
public class BackendRestorerProcedure<T extends Closeable & Disposable, S extends StateObject> {
	...
	public BackendRestorerProcedure(@Nonnull FunctionWithException<Collection<S>, T, Exception> instanceSupplier,
		@Nonnull CloseableRegistry backendCloseableRegistry, @Nonnull String logDescription) {
	...
	public T createAndRestore(@Nonnull List<? extends Collection<S>> restoreOptions) throws Exception {
		...
		while(alternativeIdx < restoreOptions.size()) {
			...
			if(restoreState.isEmpty()) {
				...
				if(LOG.isTraceEnabled()) {
					LOG.trace("Creating {} and restoring with state {} from alternative ({}/{}).", logDescription, restoreState, alternativeIdx,
						restoreOptions.size());
				} else {
					LOG.debug("Creating {} and restoring with state from alternative ({}/{}).", logDescription, alternativeIdx,
						restoreOptions.size());
				}
			}
			...
			/*************************************************
			 *
			 *  注释： 会跳转到 BackendRestorerProcedure 构建的时候，StateBackend 创建方法中
			 */
			try {
				...
				LOG.warn("Exception while restoring {} from alternative ({}/{}), will retry while more " + "alternatives are available.",
		...
		throw new FlinkException(
			"Could not restore " + logDescription + " from any of the " + restoreOptions.size() + " provided restore options.",
			collectedException);
	}
