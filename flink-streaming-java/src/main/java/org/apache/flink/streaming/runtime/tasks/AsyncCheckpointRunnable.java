	...
	public void run() {
		...
		try {
			...
			if(asyncCheckpointState.compareAndSet(AsyncCheckpointState.RUNNING, AsyncCheckpointState.COMPLETED)) {
				...
				/*************************************************
				 * 
				 *  注释： 如果 Checkpoint 执行成功，AsyncCheckpointRunnable 最后
				 *  会调用 TaskStateManagerImpl 的 reportTaskStateSnapshots 方法向 JobManager 发送 AcknowledgeCheckpoint 消息。
				 */
