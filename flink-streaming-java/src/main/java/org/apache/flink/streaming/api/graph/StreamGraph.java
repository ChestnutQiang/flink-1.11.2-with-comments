 ...
 *
 * // TODO_MA 注释： StreamGraph 然后再经过优化转换为 JobGraph， Client 端向 JobManager 提交的作业
 * // TODO_MA 注释： 就是以 JobGraph 的形式提交的
 *
 * // TODO_MA 注释： 对于 JobManager 来说，它从客户端接收的作业实际上就是一个 JobGraph，
 * // TODO_MA 注释： 然后它再对 JobGraph 做相应处理，生成具体的物理执行计划进行调度。
 *
 * // TODO_MA 注释： 表示流拓扑的类。 它包含为执行建立作业图所需的所有信息。
 ...
 *
 * // TODO_MA 注释： 根据用户通过 Stream API 编写的代码生成的最初的图
 */
@Internal
public class StreamGraph implements Pipeline {
	...
	public StreamGraph(ExecutionConfig executionConfig, CheckpointConfig checkpointConfig, SavepointRestoreSettings savepointRestoreSettings) {
		...
		// TODO_MA 注释： 确保是一个空的 StreamGraph
	...
	public <IN, OUT> void addOperator(Integer vertexID, @Nullable String slotSharingGroup, @Nullable String coLocationGroup,
		...
		/*************************************************
		 *
		 *  注释： 获取 invokableClass， 这是非常重要的信息
		 *  1、如果是 Source， 则执行： SourceStreamTask
		 *  2、如果不是 Source， 则执行：　OneInputStreamTask
		 *  到底启动类是谁，当初在 client 中构建 StreamGraph 的时候就已经指定了。
		 */
	...
	public void addVirtualPartitionNode(Integer originalId, Integer virtualId, StreamPartitioner<?> partitioner, ShuffleMode shuffleMode) {
		...
		// TODO_MA 注释： 加入 Partition 类的虚拟节点集合
