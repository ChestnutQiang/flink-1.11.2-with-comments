...
public class StreamSourceContexts {
	 ...
	 * </ul>
	 * // TODO_MA 注释： output = CountingOutput
	 */
	public static <OUT> SourceFunction.SourceContext<OUT> getSourceContext(TimeCharacteristic timeCharacteristic,
		ProcessingTimeService processingTimeService, Object checkpointLock, StreamStatusMaintainer streamStatusMaintainer,
		...
		/*************************************************
		 *
		 *  注释： 根据不同的语义获取不同的 SourceContext
		 *  1、EventTime = ManualWatermarkContext
		 *  2、IngestionTime = AutomaticWatermarkContext
		 *  3、ProcessingTime = NonTimestampContext
		 */
		switch(timeCharacteristic) {
			case EventTime:
				...
				break;
			case IngestionTime:
				ctx = new AutomaticWatermarkContext<>(output, watermarkInterval, processingTimeService, checkpointLock, streamStatusMaintainer,
					idleTimeout);
		...
		// TODO_MA 注释： 返回 WatermarkContext
		return ctx;
	}
	...
	private static class AutomaticWatermarkContext<T> extends WatermarkContext<T> {
		...
		private AutomaticWatermarkContext(final Output<StreamRecord<T>> output, final long watermarkInterval,
			final ProcessingTimeService timeService, final Object checkpointLock, final StreamStatusMaintainer streamStatusMaintainer,
			...
			this.nextWatermarkTimer = this.timeService
				.registerTimer(now + watermarkInterval, new WatermarkEmittingTask(this.timeService, checkpointLock, output));
		}
	...
	private static class ManualWatermarkContext<T> extends WatermarkContext<T> {
		...
		private ManualWatermarkContext(final Output<StreamRecord<T>> output, final ProcessingTimeService timeService,
			final Object checkpointLock, final StreamStatusMaintainer streamStatusMaintainer, final long idleTimeout) {

			super(timeService, checkpointLock, streamStatusMaintainer, idleTimeout);

			// TODO_MA 注释： CountingOutput
			this.output = Preconditions.checkNotNull(output, "The output cannot be null.");
			this.reuse = new StreamRecord<>(null);
		}
	...
	private abstract static class WatermarkContext<T> implements SourceFunction.SourceContext<T> {
		...
		private void scheduleNextIdleDetectionTask() {
			if(idleTimeout != -1) {
				...
				nextCheck = this.timeService
					.registerTimer(this.timeService.getCurrentProcessingTime() + idleTimeout, new IdlenessDetectionTask());
			}
		}
