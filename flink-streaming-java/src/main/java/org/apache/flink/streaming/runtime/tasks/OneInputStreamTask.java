...
/**
 * // TODO_MA 注释： 这是一个用来执行 OneInputStreamOperator 的 StreamTask
...
public class OneInputStreamTask<IN, OUT> extends StreamTask<OUT, OneInputStreamOperator<IN, OUT>> {
	...
	public void init() throws Exception {
		...
		if(numberOfInputs > 0) {
			...
			/*************************************************
			 * 
			 *  注释： 创建 CheckpointedInputGate
			 */
			CheckpointedInputGate inputGate = createCheckpointedInputGate();

			/*************************************************
			 * 
			 *  注释： 创建 DataOutput = StreamTaskNetworkOutput
			 */
			DataOutput<IN> output = createDataOutput();

			/*************************************************
			 * 
			 *  注释： 创建 StreamTaskInput = StreamTaskNetworkInput
			 *  因为该 StreamTask 需要从上游拉取数据，进行消费，所以默认实现是： StreamTaskNetworkInput
			 */
			StreamTaskInput<IN> input = createTaskInput(inputGate, output);

			/*************************************************
			 * 
			 *  注释： 创建 StreamOneInputProcessor
			 */
	...
	private CheckpointedInputGate createCheckpointedInputGate() {
		...
		return InputProcessorUtil.createCheckpointedInputGate(this, configuration, getCheckpointCoordinator(), inputGates,
			getEnvironment().getMetricGroup().getIOMetricGroup(), getTaskNameWithSubtaskAndId());
	}
	...
	private DataOutput<IN> createDataOutput() {
		...
		/*************************************************
		 * 
		 *  注释： 默认输出组件： StreamTaskNetworkOutput
		 */
		return new StreamTaskNetworkOutput<>(headOperator, getStreamStatusMaintainer(), inputWatermarkGauge,
	...
	private StreamTaskInput<IN> createTaskInput(CheckpointedInputGate inputGate, DataOutput<IN> output) {
		...
		/*************************************************
		 * 
		 *  注释： 最后返回： StreamTaskNetworkInput
		 */
		return new StreamTaskNetworkInput<>(inputGate, inSerializer, getEnvironment().getIOManager(), statusWatermarkValve, 0);
	}
	...
	private static class StreamTaskNetworkOutput<IN> extends AbstractDataOutput<IN> {
		...
		private StreamTaskNetworkOutput(OneInputStreamOperator<IN, ?> operator, StreamStatusMaintainer streamStatusMaintainer,
			WatermarkGauge watermarkGauge, Counter numRecordsIn) {
