...
public class StreamConfig implements Serializable {
	...
	public void setManagedMemoryFraction(double managedMemFraction) {
		checkArgument(managedMemFraction >= 0.0 && managedMemFraction <= 1.0,
	...
	public TypeSerializer<?>[] getTypeSerializersIn(ClassLoader cl) {
		...
		checkState(typeSerializersCount >= 0, "Missing value for %s in the config? [%d]", TYPE_SERIALIZERS_IN_COUNT, typeSerializersCount);
	...
	public <T> TypeSerializer<T> getTypeSerializerIn(int index, ClassLoader cl) {
		try {
			return InstantiationUtil.readObjectFromConfig(this.config, String.format(TYPE_SERIALIZERS_IN_PATTERN, index), cl);
		} catch(Exception e) {
			throw new StreamTaskException(String.format("Could not instantiate serializer for [%d] input.", index), e);
		}
	}
	...
	public <T extends StreamOperatorFactory<?>> T getStreamOperatorFactory(ClassLoader cl) {
		try {
			/*************************************************
			 *
			 *  注释： 通过配置获取： StreamOperatorFactory
			 */
	...
	public <T> List<OutputSelector<T>> getOutputSelectors(ClassLoader userCodeClassloader) {
		try {
			List<OutputSelector<T>> selectors = InstantiationUtil
				.readObjectFromConfig(this.config, OUTPUT_SELECTOR_WRAPPER, userCodeClassloader);
	...
	public boolean isUnalignedCheckpointsEnabled() {
		...
		// TODO_MA 注释： 默认： execution.checkpointing.unaligned = false
	...
	public List<StreamEdge> getOutEdgesInOrder(ClassLoader cl) {
		try {
			...
			/*************************************************
			 *
			 *  注释： 获取当前 StreamTask 的输出 SreamEdge
			 */
	...
	public StateBackend getStateBackend(ClassLoader cl) {
		try {
			...
			/*************************************************
			 *
			 *  注释： 通过 key（statebackend） 获取 StateBackend 的配置
			 */
