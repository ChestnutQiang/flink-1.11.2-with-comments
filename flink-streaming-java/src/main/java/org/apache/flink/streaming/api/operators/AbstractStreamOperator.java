...
/**
 * // TODO_MA 注释： 所有流运算符的基类。包含用户功能的运算符应扩展类 {@link AbstractUdfStreamOperator}（该类的专门子类）。
 * Base class for all stream operators. Operators that contain a user function should extend the class
 * {@link AbstractUdfStreamOperator} instead (which is a specialized subclass of this class).
 *
 * // TODO_MA 注释： 对于具体的实现，还必须实现以下两个接口之一，以*将操作符标记为一元或二进制：
 * // TODO_MA 注释： {@link OneInputStreamOperator} 或 {@link TwoInputStreamOperator}。
...
public abstract class AbstractStreamOperator<OUT> implements StreamOperator<OUT>, SetupableStreamOperator<OUT>, CheckpointedStreamOperator, Serializable {
	...
	// TODO_MA 注释： 用于收集当前 StreamOperator 处理之后的结果数据
	...
	public void setup(StreamTask<?, ?> containingTask, StreamConfig config, Output<StreamRecord<OUT>> output) {
		...
		try {
			...
			this.latencyStats = new LatencyStats(jobMetricGroup.addGroup("latency"), historySize, container.getIndexInSubtaskGroup(),
				getOperatorID(), granularity);
		} catch(Exception e) {
			LOG.warn("An error occurred while instantiating latency metrics.", e);
			this.latencyStats = new LatencyStats(UnregisteredMetricGroups.createUnregisteredTaskManagerJobMetricGroup().addGroup("latency"), 1,
				0, new OperatorID(), LatencyStats.Granularity.SINGLE);
		}

		/*************************************************
		 *
		 *  注释： 获取 StreamingRuntimeContext
		 */
		this.runtimeContext = new StreamingRuntimeContext(environment, environment.getAccumulatorRegistry().getUserMap(), getMetricGroup(),
			getOperatorID(), getProcessingTimeService(), null, environment.getExternalResourceInfoProvider());
	...
	public final void initializeState(StreamTaskStateInitializer streamTaskStateManager) throws Exception {
		...
		/*************************************************
		 *
		 *  注释： 获取 StreamOperatorStateContext， 会执行各种 state 的恢复
		 */
		...
		/*************************************************
		 *
		 *  注释： 创建 StreamOperatorStateHandler 实例对象
		 */
		...
		/*************************************************
		 *
		 *  注释： StreamOperatorStateHandler 初始化
		 */
