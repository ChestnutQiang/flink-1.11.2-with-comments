...
/**
 * // TODO_MA 注释： 用于使用 {@link SingleOutputStreamOperator＃split} 调用为 {@link SplitStream} 定义 OutputSelector 的接口。
 * Interface for defining an OutputSelector for a {@link SplitStream} using the {@link SingleOutputStreamOperator#split} call.
 *
 * // TODO_MA 注释： {@link SplitStream} 的每个输出对象都将通过此运算符运行以选择输出。
 * Every output object of a {@link SplitStream} will run through this operator to select outputs.
 *
 * @param <OUT>
