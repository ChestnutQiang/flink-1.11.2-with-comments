...
public class CheckpointBarrierAligner extends CheckpointBarrierHandler {
	...
	CheckpointBarrierAligner(String taskName, AbstractInvokable toNotifyOnCheckpoint, InputGate... inputGates) {
		...
		blockedChannels = Arrays.stream(inputGates).flatMap(gate -> gate.getChannelInfos().stream())
	...
	/*************************************************
	 *
	 *  注释： Source 节点接收到 JobManager 发送的 TriggerCheckpoint 消息触发快照，非 Source 节点接收到上游节点的 Barrier 后触发快照。
	 */
	@Override
	public void processBarrier(CheckpointBarrier receivedBarrier, InputChannelInfo channelInfo) throws Exception {
		final long barrierId = receivedBarrier.getId();

		// TODO_MA 注释： 如果只有一个 Input Channel，就不需要对齐操作了，barrierId 有效就直接触发 checkpoint
		// TODO_MA 注释： 如果只有一个 Input Channel，收到 Barrier 后直接调用 notifyCheckpoint 触发快照。
		...
		/*************************************************
		 *
		 *  注释： 如果包含多个 Input Channel， 先执行 Barrier 对齐，
		 *  收到所有 Input Channel 发送的 Barrier 后再调用 notifyCheckpoint() 触发快照。
		 */

		// TODO_MA 注释： 如果现在已经开始执行对齐
		// -- general code path for multiple input channels --
		if(isCheckpointPending()) {
			// this is only true if some alignment is already progress and was not canceled

			// TODO_MA 注释： 阻塞对应的 channel
			if(barrierId == currentCheckpointId) {
				// regular case
				onBarrier(channelInfo);
			}

			// TODO_MA 注释： 如果 barrierId > currentCheckpointId, 则跳过
		...
		/*************************************************
		 *
		 *  注释： 收到了所有 inputchannel 的 barrier，可以触发 checkpoint 了，并且要移除 channel 的阻塞标记
		 */
		...
		if(numBarriersReceived + numClosedChannels == totalNumberOfInputChannels) {
			...
			if(LOG.isDebugEnabled()) {
				LOG.debug("{}: Received all barriers, triggering checkpoint {} at {}.", taskName, receivedBarrier.getId(),
	...
	public void processCancellationBarrier(CancelCheckpointMarker cancelBarrier) throws Exception {
		...
		if(isCheckpointPending()) {
			...
			if(barrierId == currentCheckpointId) {
				...
				LOG.warn(
					"{}: Received cancellation barrier for checkpoint {} before completing current checkpoint {}. " + "Skipping current checkpoint.",
					taskName, barrierId, currentCheckpointId);
	...
	public String toString() {
		return String
			.format("%s: last checkpoint: %d, current barriers: %d, closed channels: %d", taskName, currentCheckpointId, numBarriersReceived,
				numClosedChannels);
	}
}
