...
public class StreamTaskStateInitializerImpl implements StreamTaskStateInitializer {
	 ...
	 *
	 *  注释： 总共要恢复五大类信息：
	 *  1、keyedStatedBackend
	 *  2、operatorStateBackend
	 *  3、rawKeyedStateInputs
	 *  4、rawOperatorStateInputs
	 *  5、timeServiceManager
	 *  最终，构建一个 StreamOperatorStateContextImpl 对象返回
	 */
	@Override
	public StreamOperatorStateContext streamOperatorStateContext(@Nonnull OperatorID operatorID, @Nonnull String operatorClassName,
		...
		try {
			...
			/*************************************************
			 *
			 *  注释： 获取到 keyedStatedBackend
			 */
			// -------------- Keyed State Backend --------------
			keyedStatedBackend = keyedStatedBackend(keySerializer, operatorIdentifierText, prioritizedOperatorSubtaskStates,
				streamTaskCloseableRegistry, metricGroup);

			/*************************************************
			 *
			 *  注释： 获取到 operatorStateBackend
			 */
			...
			/*************************************************
			 *
			 *  注释： 最终返回 StreamOperatorStateContextImpl
			 */
	...
	protected <K> InternalTimeServiceManager<K> internalTimeServiceManager(AbstractKeyedStateBackend<K> keyedStatedBackend,
		...
		for(KeyGroupStatePartitionStreamProvider streamProvider : rawKeyedStates) {
			...
			Preconditions
				.checkArgument(keyGroupRange.contains(keyGroupIdx), "Key Group " + keyGroupIdx + " does not belong to the local range.");
	...
	protected OperatorStateBackend operatorStateBackend(String operatorIdentifierText,
		PrioritizedOperatorSubtaskState prioritizedOperatorSubtaskStates, CloseableRegistry backendCloseableRegistry) throws Exception {
		...
		/*************************************************
		 *
		 *  注释： 创建 BackendRestorerProcedure
		 */
		BackendRestorerProcedure<OperatorStateBackend, OperatorStateHandle> backendRestorer = new BackendRestorerProcedure<>((stateHandles) ->
			/*************************************************
			 *
			 *  注释： 创建 StateBackend
			 */
			stateBackend.createOperatorStateBackend(environment, operatorIdentifierText, stateHandles, cancelStreamRegistryForRestore),
	...
	protected <K> AbstractKeyedStateBackend<K> keyedStatedBackend(TypeSerializer<K> keySerializer, String operatorIdentifierText,
		...
		/*************************************************
		 *
		 *  注释： 初始化 BackendRestorerProcedure
		 *  它的内部会创建，持有一个： KeyedStateBackend
		 */
		BackendRestorerProcedure<AbstractKeyedStateBackend<K>, KeyedStateHandle> backendRestorer = new BackendRestorerProcedure<>(
			(stateHandles) ->
				 ...
				 *  注释： 创建 StateBackend
				 */
				stateBackend.createKeyedStateBackend(environment, environment.getJobID(), operatorIdentifierText, keySerializer,
					taskInfo.getMaxNumberOfParallelSubtasks(), keyGroupRange, environment.getTaskKvStateRegistry(), ttlTimeProvider, metricGroup,
					stateHandles, cancelStreamRegistryForRestore), backendCloseableRegistry, logDescription);

		/*************************************************
		 *
		 *  注释： 恢复状态
		 *  内部会转到调用上述的 stateBackend.createKeyedStateBackend(....) 方法
		 */
		try {
	...
	private static class StreamOperatorStateContextImpl implements StreamOperatorStateContext {
		...
		StreamOperatorStateContextImpl(boolean restored, OperatorStateBackend operatorStateBackend,
			AbstractKeyedStateBackend<?> keyedStateBackend, InternalTimeServiceManager<?> internalTimeServiceManager,
