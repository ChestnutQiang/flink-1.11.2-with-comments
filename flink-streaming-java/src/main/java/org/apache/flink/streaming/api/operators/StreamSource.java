...
public class StreamSource<OUT, SRC extends SourceFunction<OUT>> extends AbstractUdfStreamOperator<OUT, SRC> {
	...
	public void run(final Object lockingObject, final StreamStatusMaintainer streamStatusMaintainer,
		final OperatorChain<?, ?> operatorChain) throws Exception {

		/*************************************************
		 *
		 *  注释： 运行 SourceOperator
		 */
	...
	public void run(final Object lockingObject, final StreamStatusMaintainer streamStatusMaintainer, final Output<StreamRecord<OUT>> collector,
		...
		// TODO_MA 注释： 查看/获取 用户设置的时间语义（ProcessingTime， IngestionTime， EventTime）
		...
		this.ctx = StreamSourceContexts
			.getSourceContext(timeCharacteristic, getProcessingTimeService(), lockingObject, streamStatusMaintainer, collector,
				watermarkInterval, -1);
	...
	public void advanceToEndOfEventTime() {
		if(!hasSentMaxWatermark) {
			...
			// TODO_MA 注释： 发送水位，让 SourceStreamTask 有执行逻辑处理的机会
	...
	private static class LatencyMarksEmitter<OUT> {
		...
		public LatencyMarksEmitter(final ProcessingTimeService processingTimeService, final Output<StreamRecord<OUT>> output,
			long latencyTrackingInterval, final OperatorID operatorId, final int subtaskIndex) {
