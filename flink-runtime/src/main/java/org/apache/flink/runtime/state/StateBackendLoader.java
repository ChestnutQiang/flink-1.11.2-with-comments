...
public class StateBackendLoader {
	 ...
	 * @throws DynamicCodeLoadingException   Thrown if a state backend factory is configured and the factory class was not
	 *                                       found or the factory could not be instantiated
	 * @throws IllegalConfigurationException May be thrown by the StateBackendFactory when creating / configuring the state
	...
	public static StateBackend loadStateBackendFromConfig(ReadableConfig config, ClassLoader classLoader,
		...
		// TODO_MA 注释： 获取到 state.backend 的配置
		...
		switch(backendName.toLowerCase()) {
			...
			// TODO_MA 注释： 如果为 jobmanager， 则创建 MemoryStateBackend
			...
			// TODO_MA 注释： 如果为 filesystem， 则创建 FsStateBackend
			...
			// TODO_MA 注释： 如果为 rocksdb， 则创建 RocksDBStateBackend
			...
			default:
				...
				try {
					@SuppressWarnings("rawtypes") Class<? extends StateBackendFactory> clazz = Class
						.forName(factoryClassName, false, classLoader).asSubclass(StateBackendFactory.class);
	 ...
	 * @throws IOException                   May be thrown by the StateBackendFactory when instantiating the state backend
	 */
	public static StateBackend fromApplicationOrConfigOrDefault(@Nullable StateBackend fromApplication, Configuration config,
		ClassLoader classLoader, @Nullable Logger logger) throws IllegalConfigurationException, DynamicCodeLoadingException, IOException {
		...
		// TODO_MA 注释： 如果 配置为空
			...
			// TODO_MA 注释： 检查配置中，是否配置了 StateBackend
			// TODO_MA 注释： 如果配置了 StateBackend， 则代码走这儿，一般企业中，用 FsStateBackend
			...
			// TODO_MA 注释： 如果也没有配置，则使用默认的： MemoryStateBackend
			else {
				// (3) use the default
