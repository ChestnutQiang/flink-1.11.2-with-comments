...
/**
 * // TODO_MA 注释： 插槽共享单元定义可以在插槽中一起部署（来自不同工作顶点的）不同任务。
 * // TODO_MA 注释： 与共置提示定义的硬约束相比，这是软许可。
 * A slot sharing units defines which different task (from different job vertices) can be deployed together within a slot.
 * This is a soft permission, in contrast to the hard constraint defined by a co-location hint.
 */
