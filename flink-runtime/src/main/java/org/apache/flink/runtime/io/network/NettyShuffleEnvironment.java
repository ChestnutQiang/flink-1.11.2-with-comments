...
/**
 * // TODO_MA 注释： {@link ShuffleEnvironment} 的实现基于 Netty 网络通信，本地内存和磁盘文件。
 * // TODO_MA 注释： 网络环境包含跟踪所有中间结果的数据结构，并进行随机数据交换。
 ...
 *
 * // TODO_MA 注释： 对应一个 SingleInputGate， 产出一个 ResultPartition
 */
public class NettyShuffleEnvironment implements ShuffleEnvironment<ResultPartition, SingleInputGate> {
	...
	/*************************************************
	 *
	 *  注释： 管理 InputGateID 和 SingleInputGate 之间的映射关系
	 */
	...
	NettyShuffleEnvironment(ResourceID taskExecutorResourceId, NettyShuffleEnvironmentConfiguration config, NetworkBufferPool networkBufferPool,
		...
		// TODO_MA 注释： NettyConnectionManager
	...
	public ShuffleIOOwnerContext createShuffleIOOwnerContext(String ownerName, ExecutionAttemptID executionAttemptID, MetricGroup parentGroup) {
		...
		// TODO_MA 注释： 创建 ShuffleIOOwnerContext
	...
	public List<ResultPartition> createResultPartitionWriters(ShuffleIOOwnerContext ownerContext,
		...
		synchronized(lock) {
			...
			for(int partitionIndex = 0; partitionIndex < resultPartitions.length; partitionIndex++) {
				...
				/*************************************************
				 *
				 *  注释： 一个 ResultParition 关联到一个 ResultPartitionWriter
				 */
	...
	public List<SingleInputGate> createInputGates(ShuffleIOOwnerContext ownerContext,
		PartitionProducerStateProvider partitionProducerStateProvider, List<InputGateDeploymentDescriptor> inputGateDeploymentDescriptors) {
		synchronized(lock) {
			Preconditions.checkState(!isClosed, "The NettyShuffleEnvironment has already been shut down.");
			...
			for(int gateIndex = 0; gateIndex < inputGates.length; gateIndex++) {
				...
				// TODO_MA 注释： 加入待返回 InputGate List
