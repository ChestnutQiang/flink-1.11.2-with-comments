...
/**
 * // TODO_MA 注释： 状态表的基类。对状态的访问通常受当前活动密钥的限制，如通过{@link InternalKeyContext}提供的*。
...
public abstract class StateTable<K, N, S> implements StateSnapshotRestore, Iterable<StateEntry<K, N, S>> {
	...
	public StateTable(InternalKeyContext<K> keyContext, RegisteredKeyValueStateBackendMetaInfo<N, S> metaInfo, TypeSerializer<K> keySerializer) {
		...
		@SuppressWarnings("unchecked") StateMap<K, N, S>[] state = (StateMap<K, N, S>[]) new StateMap[keyContext.getKeyGroupRange()
	...
	public Stream<K> getKeys(N namespace) {
		return Arrays.stream(keyGroupedStateMaps)
			...
			.filter(entry -> entry.getNamespace().equals(namespace)).map(StateEntry::getKey);
	}
	...
	public Iterator<StateEntry<K, N, S>> iterator() {
		return Arrays.stream(keyGroupedStateMaps).filter(Objects::nonNull)
			.flatMap(stateMap -> StreamSupport.stream(Spliterators.spliteratorUnknownSize(stateMap.iterator(), 0), false)).iterator();
	}
	...
	class StateEntryIterator implements StateIncrementalVisitor<K, N, S> {
		...
		private void next() {
			while(keyGroupIndex < keyGroupedStateMaps.length) {
				...
				StateIncrementalVisitor<K, N, S> visitor = stateMap.getStateIncrementalVisitor(recommendedMaxNumberOfReturnedRecords);
				if(visitor.hasNext()) {
		...
		public boolean hasNext() {
			while(stateIncrementalVisitor == null || !stateIncrementalVisitor.hasNext()) {
				...
				StateIncrementalVisitor<K, N, S> visitor = keyGroupedStateMaps[keyGroupIndex++]
