...
public class SourceCoordinatorContext<SplitT extends SourceSplit> implements SplitEnumeratorContext<SplitT>, AutoCloseable {
	...
	public SourceCoordinatorContext(ExecutorService coordinatorExecutor,
		SourceCoordinatorProvider.CoordinatorExecutorThreadFactory coordinatorThreadFactory, int numWorkerThreads,
		OperatorCoordinator.Context operatorCoordinatorContext, SimpleVersionedSerializer<SplitT> splitSerializser) {
		this(coordinatorExecutor, coordinatorThreadFactory, numWorkerThreads, operatorCoordinatorContext, splitSerializser,
			new SplitAssignmentTracker<>());
	}

	// Package private method for unit test.
	SourceCoordinatorContext(ExecutorService coordinatorExecutor,
		SourceCoordinatorProvider.CoordinatorExecutorThreadFactory coordinatorThreadFactory, int numWorkerThreads,
		OperatorCoordinator.Context operatorCoordinatorContext, SimpleVersionedSerializer<SplitT> splitSerializer,
	...
	public void sendEventToSourceReader(int subtaskId, SourceEvent event) {
		callInCoordinatorThread(() -> {
			try {
				...
				throw new FlinkRuntimeException(String.format("Failed to send event %s to subtask %d", event, subtaskId), e);
			}
		...
		callInCoordinatorThread(() -> {
			...
			for(Integer subtaskId : assignment.assignment().keySet()) {
				if(!registeredReaders.containsKey(subtaskId)) {
					throw new IllegalArgumentException(String
						.format("Cannot assign splits %s to subtask %d because the subtask is not registered.", registeredReaders.get(subtaskId),
							subtaskId));
				}
			}
			...
			assignment.assignment().forEach((id, splits) -> {
				try {
					...
					throw new FlinkRuntimeException(String.format("Failed to assign splits %s to reader %d.", splits, id), e);
				} catch(IOException e) {
	...
	void unregisterSourceReader(int subtaskId) {
		Preconditions.checkNotNull(registeredReaders.remove(subtaskId),
			String.format("Failed to unregister source reader of id %s because it is not registered.", subtaskId));
	}
