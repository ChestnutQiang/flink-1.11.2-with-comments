...
public class LocalInputChannel extends InputChannel implements BufferAvailabilityListener {
	...
	public LocalInputChannel(SingleInputGate inputGate, int channelIndex, ResultPartitionID partitionId, ResultPartitionManager partitionManager,
		TaskEventPublisher taskEventPublisher, Counter numBytesIn, Counter numBuffersIn) {

		this(inputGate, channelIndex, partitionId, partitionManager, taskEventPublisher, 0, 0, numBytesIn, numBuffersIn);
	}

	public LocalInputChannel(SingleInputGate inputGate, int channelIndex, ResultPartitionID partitionId, ResultPartitionManager partitionManager,
		TaskEventPublisher taskEventPublisher, int initialBackoff, int maxBackoff, Counter numBytesIn, Counter numBuffersIn) {
		...
		synchronized(requestLock) {
			...
			if(subpartitionView == null) {
				...
				try {
					...
					/*************************************************
					 *
					 *  注释： Local，无需网络通信，通过 ResultPartitionManager 创建一个 ResultSubpartitionView
	...
	/*************************************************
	 *
	 *  注释：  读取数据，借助 ResultSubparitionView 消费 ResultSubparition 中的苏剧
	 */
	@Override
	Optional<BufferAndAvailability> getNextBuffer() throws IOException, InterruptedException {
		...
		// TODO_MA 注释： 通过 ResultSubparitionView 获取
	...
	public void notifyDataAvailable() {
		...
		/*************************************************
		 *
		 *  注释： 回调，在 ResultSubparition 通知 ResultSubparitionView 有数据可供消费
		 */
