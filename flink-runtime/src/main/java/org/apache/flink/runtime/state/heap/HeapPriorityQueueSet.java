...
/**
 * // TODO_MA 注释： 基于{@link HeapPriorityQueue}的具有设置语义的基于堆的优先级队列。
 * A heap-based priority queue with set semantics, based on {@link HeapPriorityQueue}.
 *
 * // TODO_MA 注释： 哈希集支持该堆以进行快速包含（重复数据删除）和删除。
 * The heap is supported by hash set for fast contains (de-duplication) and deletes.
 *
 * // TODO_MA 注释： 对象识别基于{@link #equals（Object）}进行。
 * Object identification happens based on {@link #equals(Object)}.
 *
...
public class HeapPriorityQueueSet<T extends HeapPriorityQueueElement> extends HeapPriorityQueue<T> implements KeyGroupedInternalPriorityQueue<T> {
	...
	public HeapPriorityQueueSet(@Nonnull PriorityComparator<T> elementPriorityComparator, @Nonnull KeyExtractorFunction<T> keyExtractor,
		@Nonnegative int minimumCapacity, @Nonnull KeyGroupRange keyGroupRange, @Nonnegative int totalNumberOfKeyGroups) {
	...
	private HashMap<T, T> getDedupMapForElement(T element) {
		int keyGroup = KeyGroupRangeAssignment.assignToKeyGroup(keyExtractor.extractKeyFromElement(element), totalNumberOfKeyGroups);
