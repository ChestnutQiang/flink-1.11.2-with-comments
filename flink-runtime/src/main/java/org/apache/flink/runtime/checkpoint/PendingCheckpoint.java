...
public class PendingCheckpoint {
	...
	public PendingCheckpoint(JobID jobId, long checkpointId, long checkpointTimestamp,
		Map<ExecutionAttemptID, ExecutionVertex> verticesToConfirm, Collection<OperatorID> operatorCoordinatorsToConfirm,
		Collection<String> masterStateIdentifiers, CheckpointProperties props, CheckpointStorageLocation targetLocation, Executor executor,
	...
	public boolean isFullyAcknowledged() {
		...
		// TODO_MA 注释： 所有 Task ack 都收到
		return areTasksFullyAcknowledged()

			// TODO_MA 注释： CheckpointCoordinator 也已经都回复
			&& areCoordinatorsFullyAcknowledged()

			// TODO_MA 注释： MasterState 也都回复
	...
	public CompletedCheckpoint finalizeCheckpoint() throws IOException {
		...
		synchronized(lock) {
			...
			try {
				...
				/*************************************************
				 * 
				 *  注释： 构建 CompletedCheckpoint 对象用于返回
				 */
				CompletedCheckpoint completed = new CompletedCheckpoint(jobId, checkpointId, checkpointTimestamp, System.currentTimeMillis(),
				...
				if(statsCallback != null) {
					// Finalize the statsCallback and give the completed checkpoint a callback for discards.
				...
				// TODO_MA 注释： 将此待处理检查点标记为已处置，但不要删除状态
	 ...
	 */
	public TaskAcknowledgeResult acknowledgeTask(ExecutionAttemptID executionAttemptId, TaskStateSnapshot operatorSubtaskStates,
		...
		synchronized(lock) {
			...
			// TODO_MA 注释： 收到一个 Source Execution 的反馈，则从未ack的 Execution 集合 notYetAcknowledgedTasks 中移除
			final ExecutionVertex vertex = notYetAcknowledgedTasks.remove(executionAttemptId);

			// TODO_MA 注释： 将 ack 的 Source Execution 加入到 acknowledgedTasks 集合中
			if(vertex == null) {
			...
			if(operatorSubtaskStates != null) {
				...
				// TODO_MA 注释： 遍历每个 Operator
				for(OperatorIDPair operatorID : operatorIDs) {

					// TODO_MA 注释： 拿出来一个 Operator 的 State
					...
					if(operatorState == null) {
						operatorState = new OperatorState(operatorID.getGeneratedOperatorID(), vertex.getTotalNumberOfParallelSubtasks(),
			...
			if(statsCallback != null) {
				...
				SubtaskStateStats subtaskStateStats = new SubtaskStateStats(subtaskIndex, ackTimestamp, stateSize,
					metrics.getSyncDurationMillis(), metrics.getAsyncDurationMillis(), alignmentDurationMillis, checkpointStartDelayMillis);
	...
	public TaskAcknowledgeResult acknowledgeCoordinatorState(OperatorInfo coordinatorInfo, @Nullable ByteStreamStateHandle stateHandle) {
		...
		synchronized(lock) {
			...
			// TODO_MA 注释： 如果该 checkpoint 废弃，则返回 DISCARDED
			if(discarded) {
				return TaskAcknowledgeResult.DISCARDED;
			}
			...
			// TODO_MA 注释： 从 notYetAcknowledgedOperatorCoordinators 移除 Operator
			...
			// TODO_MA 注释： 将 Operator 加入到 operatorStates
			if(stateHandle != null) {
				if(operatorState == null) {
					...
					executor.execute(new Runnable() {
						...
						public void run() {
							...
							try {
								...
								LOG.warn("Could not properly dispose the private states in the pending checkpoint {} of job {}.", checkpointId,
									jobId, t);
							} finally {
	...
	public String toString() {
		return String
			.format("Pending Checkpoint %d @ %d - confirmed=%d, pending=%d", checkpointId, checkpointTimestamp, getNumberOfAcknowledgedTasks(),
