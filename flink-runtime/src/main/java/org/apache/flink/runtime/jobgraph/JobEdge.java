...
/**
 * // TODO_MA 注释： 此类表示 job grap 中的边（communication channels）
 * This class represent edges (communication channels) in a job graph.
 *
 * // TODO_MA 注释： edges 总是从 intermediate result partition 到 job vertex
 * The edges always go from an intermediate result partition to a job vertex.
 *
 * // TODO_MA 注释： edge 通过其 {@link DistributionPattern} 进行参数化。
 * An edge is parametrized with its {@link DistributionPattern}.
 *
 * // TODO_MA 注释： 代表了job graph中的一条数据传输通道。source 是 IntermediateDataSet，target 是 JobVertex。
 * // TODO_MA 注释： 即数据通过 JobEdge 由 IntermediateDataSet 传递给目标 JobVertex
 */
