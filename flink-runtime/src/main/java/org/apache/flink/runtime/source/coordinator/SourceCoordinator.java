	...
	public void handleEventFromOperator(int subtask, OperatorEvent event) throws Exception {
		...
		coordinatorExecutor.execute(() -> {
			try {
				...
				LOG.error("Failing the job due to exception when handling operator event {} from subtask {} " + "of source {}.", event, subtask,
					operatorName, e);
				context.failJob(e);
			}
		});
	...
	public void subtaskFailed(int subtaskId, @Nullable Throwable reason) {
		...
		coordinatorExecutor.execute(() -> {
			try {
				...
				LOG.error("Failing the job due to exception when handling subtask {} failure in source {}.", subtaskId, operatorName, e);
				context.failJob(e);
			}
		});
	}
		...
		coordinatorExecutor.execute(() -> {
			try {
				...
				result.completeExceptionally(
					new CompletionException(String.format("Failed to checkpoint coordinator for source %s due to ", operatorName), e));
			}
		});
	}
	...
	public void checkpointComplete(long checkpointId) {
		...
		coordinatorExecutor.execute(() -> {
			try {
				...
				/*************************************************
				 *
				 *  注释：  checkpintID 的 checkpoint 成功了。
				 */
				context.onCheckpointComplete(checkpointId);
			} catch(Exception e) {
				LOG.error("Failing the job due to exception when completing the checkpoint {} for source {}.", checkpointId, operatorName, e);
				context.failJob(e);
			}
		});
	}
	...
	public void resetToCheckpoint(byte[] checkpointData) throws Exception {
		if(started) {
			throw new IllegalStateException(String.format(
				"The coordinator for source %s has started. The source coordinator state can " + "only be reset to a checkpoint before it starts.",
				operatorName));
		}
