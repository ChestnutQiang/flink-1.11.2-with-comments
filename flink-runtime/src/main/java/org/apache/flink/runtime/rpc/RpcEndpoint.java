...
/*************************************************
 *
 *  注释： RpcEndpoint 是对 RPC 框架中提供具体服务的实体的抽象，所有提供远程调用方法的组件都需要继承该抽象类。
 *  另外，对于同一个 RpcEndpoint 的所有 RPC 调用都会在同一个线程（RpcEndpoint 的“主线程”）中执行，因此无需担心并发执行的线程安全问题。
 */
public abstract class RpcEndpoint implements RpcGateway, AutoCloseableAsync {
	...
	/**
	 * // TODO_MA 注释： 用于启动RPC服务器和获取rpc网关的RPC服务。
