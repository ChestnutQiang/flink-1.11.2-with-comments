 ...
 * // TODO_MA 注释： Execution被调度后在分配的 TaskManager 中启动对应的 Task。Task 包裹了具有用户执行逻辑的 operator
 */
public class Task implements Runnable, TaskSlotPayload, TaskActions, PartitionProducerStateProvider, CheckpointListener, BackPressureSampleableTask {
	...
	/*************************************************
	 *
	 *  注释： 当前 Task 可能要输出多个 ResultPartition
	 */
	...
	/**
	 * // TODO_MA 注释： 在初始化  TaskExecutor 之前，就已经初始化了。
	 * <p><b>IMPORTANT:</b> This constructor may not start any work that would need to
	 * be undone in the case of a failing task deployment.</p>
	 */
	public Task(JobInformation jobInformation, TaskInformation taskInformation, ExecutionAttemptID executionAttemptID,
		AllocationID slotAllocationId, int subtaskIndex, int attemptNumber,
		...
		/*************************************************
		 *
		 *  注释： 创建 ShuffleIOOwnerContext
		 *  这个变量，在 TaskExecutor 初始化的时候，就已经创建好了
		 */
		...
		/*************************************************
		 *
		 *  注释： 初始化 ResultPartitioner
		 *  具体实现是： ResultPartition and ResultSubPartition
		 *  一般来说，一个 Task 就只有一个 ResultPartition
		 */
	...
	private void doRun() {
		...
		/*************************************************
		 *
		 *  注释： 状态处理，当能把 Task 的状态改成 DEPLOYING 的时候，就退出这个 while(true) 循环。
		 */
		// ----------------------------
		...
		try {
			...
			/*************************************************
			 *
			 *  注释： 注册 ResultPartitionWriter
			 */
			for(ResultPartitionWriter partitionWriter : consumableNotifyingPartitionWriters) {
				taskEventDispatcher.registerPartition(partitionWriter.getPartitionId());
			}

			// TODO_MA 注释： 接下来，为分布式缓存启动文件的后台复制
			// next, kick off the background copying of files for the distributed cache
			try {
				for(Map.Entry<String, DistributedCache.DistributedCacheEntry> entry : DistributedCache
					.readFileInfoFromConfig(jobConfiguration)) {
			 ...
			 *  注释： 构建一个环境对象
			 */
			Environment env = new RuntimeEnvironment(jobId, vertexId, executionId, executionConfig, taskInfo, jobConfiguration,
				taskConfiguration, userCodeClassLoader, memoryManager, ioManager, broadcastVariableManager, taskStateManager, aggregateManager,
				accumulatorRegistry, kvStateRegistry, inputSplitProvider, distributedCacheEntries, consumableNotifyingPartitionWriters,
				inputGates, taskEventDispatcher, checkpointResponder, operatorCoordinatorEventGateway, taskManagerConfig, metrics, this,
			...
			try {
				...
				// TODO_MA 注释： 释放 Task 运行需要的一些文件资源
	...
	public static void setupPartitionsAndGates(ResultPartitionWriter[] producedPartitions, InputGate[] inputGates) throws IOException {
		...
		/*************************************************
		 *
		 *  注释： 注册当前 Task 的 ResultPartition 到启动当前 Task 的 TaskManager
		...
		/*************************************************
		 *
		 *  注释： 为这个 Task 的 InputGate 中的 InputChannel 分配 BufferPool
		 */
		...
		while(true) {
			...
			if(current == ExecutionState.DEPLOYING || current == ExecutionState.CREATED) {
				...
				throw new IllegalStateException(
					String.format("Unexpected state: %s of task %s (%s).", current, taskNameWithSubtask, executionId));
			}
		}
	}
	...
	public void requestPartitionProducerState(final IntermediateDataSetID intermediateDataSetId, final ResultPartitionID resultPartitionId,
		...
		FutureUtils.assertNoException(
			futurePartitionState.handle(PartitionProducerStateResponseHandle::new).thenAcceptAsync(responseConsumer, executor));
	}
	...
	public void triggerCheckpointBarrier(final long checkpointID, final long checkpointTimestamp, final CheckpointOptions checkpointOptions,
		...
		// TODO_MA 注释： invokable = SourceStreamTask
		final AbstractInvokable invokable = this.invokable;

		// TODO_MA 注释： CheckpointMetaData 包含 checkpointID 和 checkpoint 时间戳
		final CheckpointMetaData checkpointMetaData = new CheckpointMetaData(checkpointID, checkpointTimestamp);

		/*************************************************
		 *
		 *  注释： 如果是 Task 正在运行
		 *  这里依然执行判断， SourceStreamTask 的运行状态，必须是 RUNNING 状态， 而且 invokable 不能为空
		 */
		if(executionState == ExecutionState.RUNNING && invokable != null) {
			try {
				...
				if(getExecutionState() == ExecutionState.RUNNING) {
					...
					LOG.debug("Encountered error while triggering checkpoint {} for " + "{} ({}) while being not in state running.",
						checkpointID, taskNameWithSubtask, executionId, t);
				}
			}
		} else {
	...
	public void notifyCheckpointComplete(final long checkpointID) {
		...
		if(executionState == ExecutionState.RUNNING && invokable != null) {
			try {
				...
				// This may happen if the mailbox is closed. It means that the task is shutting down, so we just ignore it.
				LOG.debug("Notify checkpoint complete {} for {} ({}) was rejected by the mailbox", checkpointID, taskNameWithSubtask,
					executionId);
			} catch(Throwable t) {
				if(getExecutionState() == ExecutionState.RUNNING) {
	...
	private static AbstractInvokable loadAndInstantiateInvokable(ClassLoader classLoader, String className,
		...
		/*************************************************
		 *
		 *  注释： 获取每个 ExecutorVertex 的 Operator
		 */
		...
		/*************************************************
		 *
		 *  注释： 注意这个地方，调用 AbstractInvokable 的带一个 RuntimeEnvironment 参数的构造
		 *  1、SourceStreamTask 的 带 RuntimeEnvironment 的构造方法
		 *  2、OneInputStreamTask 的 带 RuntimeEnvironment 的构造方法
		 */
