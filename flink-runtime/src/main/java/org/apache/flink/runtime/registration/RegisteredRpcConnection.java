...
public abstract class RegisteredRpcConnection<F extends Serializable, G extends RpcGateway, S extends RegistrationResponse.Success> {
	...
	public void start() {
		...
		/*************************************************
		 *
		 *  注释： 创建注册对象， 并且在创建成功之后，进行 Slot 汇报
		 *  返回的结果是：
		 ...
		 *  －
		 *  重点：
		 *  1、创建 注册对象： TaskExecutorToResourceManagerConnection.ResourceManagerRegistration
		 *  2、进行 Slot 汇报： onRegistrationSuccess(result.f1);
		 */
	...
	private RetryingRegistration<F, G, S> createNewRegistration() {
		...
		/*************************************************
		 *
		 *  注释： TaskExecutor 注册成功之后，会进行 Slot 汇报
		 */
