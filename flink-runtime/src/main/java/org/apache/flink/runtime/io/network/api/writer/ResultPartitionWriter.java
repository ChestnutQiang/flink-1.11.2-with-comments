...
/**
 * // TODO_MA 注释： 面向缓冲区的运行时结果编写器API，用于产生结果。
 * A buffer-oriented runtime result writer API for producing results.
 *
 * // TODO_MA 注释： 如果在{@link ResultPartitionWriter＃fail（Throwable）}或* {@link ResultPartitionWriter＃finish（）}
 * // TODO_MA 注释： 之前调用{@link ResultPartitionWriter＃close（）}，则会突然触发失败并取消生产。
 * // TODO_MA 注释： 在这种情况下，之后仍需要调用{@link ResultPartitionWriter＃fail（Throwable）}以完全释放
 * // TODO_MA 注释： 与该分区关联的所有资源，并在可能的情况下将失败原因传播给使用
...
public interface ResultPartitionWriter extends AutoCloseable, AvailabilityProvider {
	...
	/**
	 * // TODO_MA 注释： 设置分区（可能很重）会阻塞操作，而不仅仅是创建。
