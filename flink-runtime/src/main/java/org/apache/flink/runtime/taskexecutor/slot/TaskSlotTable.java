 ...
 * // TODO_MA 注释： 当一个 slot 被调度执行了一个 Task, 就会生成一个 TaskSlot 对象
 * // TODO_MA 注释： 关于 Slot 的管理： SlotManagerImpl 主节点启动的时候，已经启动了这个组件
 *
 * // TODO_MA 注释： 多个{@link TaskSlot}实例的容器。另外，它维护多个索引，以便更快地访问任务和分配的插槽组。
