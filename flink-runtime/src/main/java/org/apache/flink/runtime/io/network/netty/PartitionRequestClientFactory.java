	...
	NettyPartitionRequestClient createPartitionRequestClient(ConnectionID connectionId) throws IOException, InterruptedException {
		...
		while(client == null) {
			...
			if(entry != null) {
				...
				// TODO_MA 注释： 连接创建成功后会回调 handInChannel 方法
				...
				if(old == null) {
					...
					try {
						...
						/*************************************************
						 *
						 *  注释： NettyClient 链接对方 Task 的 NettyServer
						 */
	...
	private static final class ConnectingChannel implements ChannelFutureListener {
		...
		private void handInChannel(Channel channel) {
			synchronized(connectLock) {
				try {
					...
					partitionRequestClient = new NettyPartitionRequestClient(channel, clientHandler, connectionId, clientFactory);
		...
		public void operationComplete(ChannelFuture future) throws Exception {
			if(future.isSuccess()) {
				...
				notifyOfError(new RemoteTransportException("Connecting to remote task manager + '" + connectionId
				...
				notifyOfError(new LocalTransportException(
					String.format("Connecting to remote task manager '%s' has been cancelled.", connectionId.getAddress()), null));
			}
		}
	}
}
