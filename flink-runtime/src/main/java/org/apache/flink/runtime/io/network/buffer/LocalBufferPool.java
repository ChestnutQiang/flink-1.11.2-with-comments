...
/**
 * // TODO_MA 注释： 缓冲池，用于管理{@link NetworkBufferPool}中的多个{@link Buffer}实例
 ...
 * recycling, which ensures that every buffer is ultimately returned to the network buffer pool.
 *
 * // TODO_MA 注释： 该池的大小可以在运行时动态更改（{@link #setNumBuffers（int）}。
 * // TODO_MA 注释： 然后，它将所需数量的缓冲区懒惰地返回到 {@link NetworkBufferPool} 以匹配其新大小。
 * <p>The size of this pool can be dynamically changed at runtime ({@link #setNumBuffers(int)}. It
 * will then lazily return the required number of buffers to the {@link NetworkBufferPool} to match its new size.
 */
class LocalBufferPool implements BufferPool {
	 ...
	 */
	LocalBufferPool(NetworkBufferPool networkBufferPool, int numberOfRequiredMemorySegments, int maxNumberOfMemorySegments,
		@Nullable BufferPoolOwner bufferPoolOwner, int numberOfSubpartitions, int maxBuffersPerChannel) {
		checkArgument(maxNumberOfMemorySegments >= numberOfRequiredMemorySegments,
			"Maximum number of memory segments (%s) should not be smaller than minimum (%s).", maxNumberOfMemorySegments,
			numberOfRequiredMemorySegments);

		checkArgument(maxNumberOfMemorySegments > 0, "Maximum number of memory segments (%s) should be larger than 0.",
			maxNumberOfMemorySegments);

		LOG.debug("Using a local buffer pool with {}-{} buffers", numberOfRequiredMemorySegments, maxNumberOfMemorySegments);
		...
		if(numberOfSubpartitions > 0) {
			checkArgument(maxBuffersPerChannel > 0, "Maximum number of buffers for each channel (%s) should be larger than 0.",
	...
	public void setNumBuffers(int numBuffers) throws IOException {
		...
		CompletableFuture<?> toNotify = null;
		synchronized(availableMemorySegments) {
			checkArgument(numBuffers >= numberOfRequiredMemorySegments, "Buffer pool needs at least %s buffers, but tried to set to %s",
	...
	public String toString() {
		synchronized(availableMemorySegments) {
			return String.format(
				"[size: %d, required: %d, requested: %d, available: %d, max: %d, listeners: %d," + "subpartitions: %d, maxBuffersPerChannel: %d, destroyed: %s]",
				currentPoolSize, numberOfRequiredMemorySegments, numberOfRequestedMemorySegments, availableMemorySegments.size(),
				maxNumberOfMemorySegments, registeredListeners.size(), subpartitionBuffersCount.length, maxBuffersPerChannel, isDestroyed);
		}
	}
