...
public class CheckpointStatsHistory implements Serializable {
	...
	private CheckpointStatsHistory(boolean readOnly, int maxSize, AbstractCheckpointStats[] checkpointArray,
		...
		@Nullable CompletedCheckpointStats latestCompletedCheckpoint, @Nullable FailedCheckpointStats latestFailedCheckpoint,
	...
	CheckpointStatsHistory createSnapshot() {
		...
		return new CheckpointStatsHistory(true, maxSize, null, checkpointsHistory, checkpointsById, latestCompletedCheckpoint,
			latestFailedCheckpoint, latestSavepoint);
	}
	...
	boolean replacePendingCheckpointById(AbstractCheckpointStats completedOrFailed) {
		...
		if(completedOrFailed.getStatus().isCompleted()) {
			...
			if(completed.getProperties().isSavepoint() && (latestSavepoint == null || completed.getCheckpointId() > latestSavepoint
				.getCheckpointId())) {

				latestSavepoint = completed;
			} else if(latestCompletedCheckpoint == null || completed.getCheckpointId() > latestCompletedCheckpoint.getCheckpointId()) {
