...
/**
 * // TODO_MA 注释： 接收{@link LocalRpcInvocation}，{@ link RunAsync}和{@link CallAsync} * {@link ControlMessages}消息的Akka rpc actor。
 * Akka rpc actor which receives {@link LocalRpcInvocation}, {@link RunAsync} and {@link CallAsync}
 * {@link ControlMessages} messages.
 *
 * // TODO_MA 注释： {@link LocalRpcInvocation}指定一个rpc，并分配给给定的{@link RpcEndpoint}实例。
 * <p>The {@link LocalRpcInvocation} designates a rpc and is dispatched to the given {@link RpcEndpoint} instance.
 *
 * // TODO_MA 注释： {@link RunAsync}和{@link CallAsync}消息包含在actor线程的上下文中执行的可执行代码。
 * <p>The {@link RunAsync} and {@link CallAsync} messages contain executable code which is executed
 * in the context of the actor thread.
 *
 * // TODO_MA 注释： {@link ControlMessages}消息控制akka rpc actor的处理行为。
 * // TODO_MA 注释： {@link ControlMessages＃START}开始处理传入的消息。
 * // TODO_MA 注释： {@link ControlMessages＃STOP}消息*停止处理消息。
 * // TODO_MA 注释： 当处理停止时到达的所有消息将被丢弃。
...
class AkkaRpcActor<T extends RpcEndpoint & RpcGateway> extends AbstractActor {
	...
	AkkaRpcActor(final T rpcEndpoint, final CompletableFuture<Boolean> terminationFuture, final int version, final long maximumFramesize) {
		...
		this.rpcEndpointTerminationResult = RpcEndpointTerminationResult
			.failure(new AkkaRpcException(String.format("RpcEndpoint %s has not been properly stopped.", rpcEndpoint.getEndpointId())));
	...
	public Receive createReceive() {
		return ReceiveBuilder.create().match(RemoteHandshakeMessage.class, this::handleHandshakeMessage)
			.match(ControlMessages.class, this::handleControlMessage).matchAny(this::handleMessage).build();
	}
	...
	protected void handleRpcMessage(Object message) {
		if(message instanceof RunAsync) {
			...
			sendErrorIfSender(
				new AkkaUnknownMessageException("Received unknown message " + message + " of type " + message.getClass().getSimpleName() + '.'));
		}
	}
	...
	private void handleHandshakeMessage(RemoteHandshakeMessage handshakeMessage) {
		if(!isCompatibleVersion(handshakeMessage.getVersion())) {
			...
			sendErrorIfSender(new AkkaHandshakeException(
				String.format("The rpc endpoint does not support the gateway %s.", handshakeMessage.getRpcGateway().getSimpleName())));
		} else {
		...
		public State terminate(AkkaRpcActor<?> akkaRpcActor) {
			...
			try {
				...
				terminationFuture = FutureUtils.completedExceptionally(
					new AkkaRpcException(String.format("Failure while stopping RpcEndpoint %s.", akkaRpcActor.rpcEndpoint.getEndpointId()), t));
			} finally {
	...
	enum StoppedState implements State {
		...
		public State start(AkkaRpcActor<?> akkaRpcActor) {
			...
			try {
				...
				akkaRpcActor.stop(RpcEndpointTerminationResult.failure(
					new AkkaRpcException(String.format("Could not start RpcEndpoint %s.", akkaRpcActor.rpcEndpoint.getEndpointId()),
						throwable)));
			} finally {
				akkaRpcActor.mainThreadValidator.exitMainThread();
			}
