 ...
 * 
 *  注释：
 *  FsStateBackend 需要配置一个 checkpoint 路径，我们一般配置为 hdfs 目录
 *  FsStateBackend 将工作状态数据保存在 taskmanager 的 java 内存中。
 *  进行快照时，再将快照数据写入上面配置的路径，然后将写入的文件路径告知 jobmanager。
 *  jobmanager 中保存所有状态的元数据信息(在 HA 模式下，元数据会写入 checkpoint 目录)。
 *  FsStateBackend 默认使用异步方式进行快照，防止阻塞主线程的 pipline 处理。
 *  -
 *  FsStateBackend 适合的场景：
 *  1、大状态、长窗口、大键值（键或者值很大）状态的作业
 *  2、适合高可用方案
 */
@PublicEvolving
public class FsStateBackend extends AbstractFileStateBackend implements ConfigurableStateBackend {
	 ...
	 */
	public FsStateBackend(URI checkpointDirectory, @Nullable URI defaultSavepointDirectory, int fileStateSizeThreshold, int writeBufferSize,
	...
	private FsStateBackend(FsStateBackend original, ReadableConfig configuration, ClassLoader classLoader) {
		...
		this.asynchronousSnapshots = original.asynchronousSnapshots.resolveUndefined(configuration.get(CheckpointingOptions.ASYNC_SNAPSHOTS));

		if(getValidFileStateThreshold(original.fileStateThreshold) >= 0) {
			this.fileStateThreshold = original.fileStateThreshold;
		} else {
			final int configuredStateThreshold = getValidFileStateThreshold(configuration.get(FS_SMALL_FILE_THRESHOLD).getBytes());
			...
			if(configuredStateThreshold >= 0) {
				...
				LoggerFactory.getLogger(AbstractFileStateBackend.class)
					.warn("Ignoring invalid file size threshold value ({}): {} - using default value {} instead.", FS_SMALL_FILE_THRESHOLD.key(),
						configuration.get(FS_SMALL_FILE_THRESHOLD).getBytes(), FS_SMALL_FILE_THRESHOLD.defaultValue());
			}
		}
	...
	public CheckpointStorage createCheckpointStorage(JobID jobId) throws IOException {
		...
		/*************************************************
		 * 
		 *  注释： 创建一个 FsCheckpointStorage
		 *  可以把一个job的所有Task的状态，在某一次checkpoint的时候，保存到 fs中，一般来说就是 HDFS
		 */
		return new FsCheckpointStorage(getCheckpointPath(), getSavepointPath(), jobId, getMinFileSizeThreshold(), getWriteBufferSize());
	}
	...
	public <K> AbstractKeyedStateBackend<K> createKeyedStateBackend(Environment env, JobID jobID, String operatorIdentifier,
		...
		/*************************************************
		 * 
		 *  注释： 创建 HeapPriorityQueueSetFactory 实例
		 */
		HeapPriorityQueueSetFactory priorityQueueSetFactory = new HeapPriorityQueueSetFactory(keyGroupRange, numberOfKeyGroups, 128);

		/*************************************************
		 * 
		 *  注释： 得到一个 HeapKeyedStateBackend 实例
		 */
		return new HeapKeyedStateBackendBuilder<>(kvStateRegistry, keySerializer, env.getUserClassLoader(), numberOfKeyGroups, keyGroupRange,
			env.getExecutionConfig(), ttlTimeProvider, stateHandles, AbstractStateBackend.getCompressionDecorator(env.getExecutionConfig()),
	...
	public OperatorStateBackend createOperatorStateBackend(Environment env, String operatorIdentifier,
		@Nonnull Collection<OperatorStateHandle> stateHandles, CloseableRegistry cancelStreamRegistry) throws BackendBuildingException {

		/*************************************************
		 * 
		 *  注释： 构建一个 DefaultOperatorStateBackend
		 */
		return new DefaultOperatorStateBackendBuilder(env.getUserClassLoader(), env.getExecutionConfig(), isUsingAsynchronousSnapshots(),
