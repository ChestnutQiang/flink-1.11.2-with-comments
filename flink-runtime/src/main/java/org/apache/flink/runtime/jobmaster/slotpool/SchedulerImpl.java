...
/**
 * // TODO_MA 注释： 将任务分配给插槽的调度程序。 该课程目前正在进行中，评论将随着我们的前进而更新。
...
public class SchedulerImpl implements Scheduler {
	...
	private void internalAllocateSlot(CompletableFuture<LogicalSlot> allocationResultFuture, SlotRequestId slotRequestId, ScheduledUnit scheduledUnit,
		...
		// TODO_MA 注释： 如果没有指定 SlotSharingGroupId，说明这个任务不运行 slot 共享，要独占一个 slot
		// TODO_MA 注释： 如果设置的有 SlotSharingGroup， 则调用： allocateSharedSlot 申请 slot
	 ...
	 *
	 *  注释： 申请 SingleSlot
	 *  1、首先尝试从 JobMaster 的 SlotPool 中申请，看看是否能申请到，如果能，则进行分配即可
	 *  2、如果不能，则尝试从 ResourceManager 去申请
	 *  在一个 JobMaster 工作期间，它所申请到的所有的 Slot 都被管理在 SlotPool
	 *  JobMaster 委托 SlotPool 来进行 Slot 管理
	 */
	private CompletableFuture<LogicalSlot> allocateSingleSlot(SlotRequestId slotRequestId, SlotProfile slotProfile, @Nullable Time allocationTimeout) {
		...
		if(slotAndLocality.isPresent()) {
			// TODO_MA 注释： 如果有已经有可用的了，就创建一个 SingleLogicalSlot，并作为 AllocatedSlot 的payload
			 ...
			 *  如果你是第一次来申请 slot，那么你的 slotpool 一定是没有的的
			 *  如果你去申请的时候，可以从 slotpool 中申请，那么就不走这儿
			 */
			...
			return requestNewAllocatedSlot(slotRequestId, slotProfile, allocationTimeout).thenApply((PhysicalSlot allocatedSlot) -> {
				try {
					// TODO_MA 注释： 封装成 SlotAndLocality 返回
