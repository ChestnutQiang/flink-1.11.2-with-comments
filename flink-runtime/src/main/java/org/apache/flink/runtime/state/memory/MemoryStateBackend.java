...
/*************************************************
 * 
 *  注释： MemoryStateBackend 将工作状态数据保存在 taskmanager 的 java 内存中。
 *  key/value 状态和 window 算子使用哈希表存储数值和触发器。
 *  进行快照时（checkpointing），生成的快照数据将和 checkpoint ACK 消息一起发送给 jobmanager，
 ...
 *  1、每个 state 的默认大小被限制为 5 MB（这个值可以通过 MemoryStateBackend 构造函数设置）
 *  2、每个 task 的所有 state 数据 (一个 task 可能包含一个 pipline 中的多个 Operator) 大小不能超过 RPC 系统的帧大小(akka.framesize，默认 10MB)
 *  3、jobmanager 收到的 state 数据总和不能超过 jobmanager 内存
 *  -
 *  MemoryStateBackend 适合的场景
 *  1、本地开发和调试
 *  2、状态很小的作业
 *  -
 *  注意：MemoryStateBackend 当触发 savepoint 时，jobmanager 会把快照数据持久化到外部存储。
 */
@PublicEvolving
public class MemoryStateBackend extends AbstractFileStateBackend implements ConfigurableStateBackend {
	...
	public CheckpointStorage createCheckpointStorage(JobID jobId) throws IOException {
		...
		/*************************************************
		 * 
		 *  注释： 创建 MemoryBackendCheckpointStorage
		 */
	...
	public <K> AbstractKeyedStateBackend<K> createKeyedStateBackend(Environment env, JobID jobID, String operatorIdentifier,
		...
		/*************************************************
		 * 
		 *  注释： 创建一个 HeapKeyedStateBackend
		 *  -
		 *  HeapKeyedStateBackend 会提供创建各种 State 的接口（ValueState，ListState，MapState，
		 *  SortedMapState，ReducingState，FoldingState，AggregatingState，RawQueueState，
		 *  RawSecodaryState，RawStortedSecondaryState 等），
		 *  返回当前 state 的 snapshot（checkpoint 会用到） 以及从指定的 KeyedStateHandles 恢复的接口。
		 */
		return new HeapKeyedStateBackendBuilder<>(kvStateRegistry, keySerializer, env.getUserClassLoader(), numberOfKeyGroups, keyGroupRange,
			...
			taskStateManager.createLocalRecoveryConfig(), priorityQueueSetFactory, isUsingAsynchronousSnapshots(), cancelStreamRegistry)
			.build();
	}
