...
public class ZooKeeperCompletedCheckpointStore implements CompletedCheckpointStore {
	...
	private static final Comparator<Tuple2<RetrievableStateHandle<CompletedCheckpoint>, String>> STRING_COMPARATOR = Comparator
		.comparing(o -> o.f1);
	...
	public ZooKeeperCompletedCheckpointStore(int maxNumberOfCheckpointsToRetain,
		ZooKeeperStateHandleStore<CompletedCheckpoint> checkpointsInZooKeeper, Executor executor) {
	...
	public void recover() throws Exception {
		...
		} while(retrievedCheckpoints.size() != numberOfInitialCheckpoints && !CompletedCheckpoint
			.checkpointsMatch(lastTryRetrievedCheckpoints, retrievedCheckpoints));
		...
		if(completedCheckpoints.isEmpty() && numberOfInitialCheckpoints > 0) {
			...
			LOG.warn("Could only fetch {} of {} checkpoints from storage.", completedCheckpoints.size(), numberOfInitialCheckpoints);
		}
	}
	...
	private void tryRemoveCompletedCheckpoint(CompletedCheckpoint completedCheckpoint,
	...
	public void shutdown(JobStatus jobStatus) throws Exception {
		if(jobStatus.isGloballyTerminalState()) {
			...
			for(CompletedCheckpoint checkpoint : completedCheckpoints) {
				tryRemoveCompletedCheckpoint(checkpoint, completedCheckpoint -> completedCheckpoint.discardOnShutdown(jobStatus));
			}
	...
	public static long pathToCheckpointId(String path) {
		try {
			...
			LOG.warn("Could not parse checkpoint id from {}. This indicates that the " + "checkpoint id to path conversion has changed.", path);
	...
	private static CompletedCheckpoint retrieveCompletedCheckpoint(
		...
		try {
			...
			throw new FlinkException(
				"Could not retrieve checkpoint " + checkpointId + " from state handle under " + stateHandlePath.f1 + ". This indicates that you are trying to recover from state written by an " + "older Flink version which is not compatible. Try cleaning the state handle store.",
				cnfe);
		} catch(IOException ioe) {
			throw new FlinkException(
				"Could not retrieve checkpoint " + checkpointId + " from state handle under " + stateHandlePath.f1 + ". This indicates that the retrieved state handle is broken. Try cleaning the " + "state handle store.",
				ioe);
		}
	}
}
