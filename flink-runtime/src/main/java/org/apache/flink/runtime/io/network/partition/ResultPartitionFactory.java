...
public class ResultPartitionFactory {
	...
	public ResultPartitionFactory(ResultPartitionManager partitionManager, FileChannelManager channelManager,
		BufferPoolFactory bufferPoolFactory, BoundedBlockingSubpartitionType blockingSubpartitionType, int networkBuffersPerChannel,
		int floatingNetworkBuffersPerGate, int networkBufferSize, boolean forcePartitionReleaseOnConsumption,
	...
	public ResultPartition create(String taskNameWithSubtaskAndId, int partitionIndex, ResultPartitionDeploymentDescriptor desc) {
		...
		// TODO_MA 注释： 创建 ResultPartition
		return create(taskNameWithSubtaskAndId, partitionIndex, desc.getShuffleDescriptor().getResultPartitionID(), desc.getPartitionType(),
			desc.getNumberOfSubpartitions(), desc.getMaxParallelism(),
			createBufferPoolFactory(desc.getNumberOfSubpartitions(), desc.getPartitionType()));
	}

	@VisibleForTesting
	public ResultPartition create(String taskNameWithSubtaskAndId, int partitionIndex, ResultPartitionID id, ResultPartitionType type,
		int numberOfSubpartitions, int maxParallelism, FunctionWithException<BufferPoolOwner, BufferPool, IOException> bufferPoolFactory) {
		...
		// TODO_MA 注释： 初始化一个 ResultSubpartition 数组
		ResultSubpartition[] subpartitions = new ResultSubpartition[numberOfSubpartitions];

		/*************************************************
		 * 
		 *  注释： 创建 ResultPartition 实例
		 */
		ResultPartition partition = forcePartitionReleaseOnConsumption || !type.isBlocking() ?

			// TODO_MA 注释： 批处理，创建一个 ReleaseOnConsumptionResultPartition
			new ReleaseOnConsumptionResultPartition(taskNameWithSubtaskAndId, partitionIndex, id, type,
				subpartitions, maxParallelism, partitionManager, bufferCompressor, bufferPoolFactory) :

			// TODO_MA 注释： 流式处理，创建一个 ResultPartition
			new ResultPartition(taskNameWithSubtaskAndId, partitionIndex, id, type, subpartitions, maxParallelism, partitionManager,
				bufferCompressor, bufferPoolFactory);

		/*************************************************
		 * 
		 *  注释： 创建 ResultSubPartition
		 */
	...
	private void createSubpartitions(ResultPartition partition, ResultPartitionType type,
		BoundedBlockingSubpartitionType blockingSubpartitionType, ResultSubpartition[] subpartitions) {
		...
		if(type.isBlocking()) {
			 ...
			 * 
			 *  注释： 流分区
			 *  subpartitions.length = 下游 Task 的个数
			 */
			for(int i = 0; i < subpartitions.length; i++) {

				// TODO_MA 注释： 创建 PipelinedSubpartition ResultSubpartition
				subpartitions[i] = new PipelinedSubpartition(i, partition);
			}
		}
	}

	private static void initializeBoundedBlockingPartitions(ResultSubpartition[] subpartitions, ResultPartition parent,
		BoundedBlockingSubpartitionType blockingSubpartitionType, int networkBufferSize, FileChannelManager channelManager) {
		int i = 0;
		try {
	 ...
	 */
	@VisibleForTesting
	FunctionWithException<BufferPoolOwner, BufferPool, IOException> createBufferPoolFactory(int numberOfSubpartitions,
		ResultPartitionType type) {
		return bufferPoolOwner -> {

			/*************************************************
			 * 
			 *  注释： 如果PartitionType 是 unbounded，则不限制 buffer pool 的最大大小，
			 *  否则为 sub-partition * taskmanager.network.memory.buffers-per-channel
			 */
			...
			// TODO_MA 注释： 创建一个 LocalBufferPool，请求的最少的 MemeorySegment 数量和 sub-partition 一致
			// TODO_MA 注释： 如果没有反压，则需要自己处理 buffer 的回收（主要是在batch模式）
			// If the partition type is back pressure-free, we register with the buffer pool for
			// callbacks to release memory.
			return bufferPoolFactory
				.createBufferPool(numberOfSubpartitions + 1, maxNumberOfMemorySegments, type.hasBackPressure() ? null : bufferPoolOwner,
