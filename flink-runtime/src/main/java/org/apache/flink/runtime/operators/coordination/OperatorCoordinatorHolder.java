...
public class OperatorCoordinatorHolder implements OperatorCoordinator, OperatorCoordinatorCheckpointContext {
	...
	public void checkpointCoordinator(long checkpointId, CompletableFuture<byte[]> result) {
		...
		/*************************************************
		 * 
		 *  注释： 不幸的是，此方法不是在调度程序执行器中运行，而是在检查点协调器时间线程中运行。
		 *  一旦 checkpoint coordinator 在调度程序的主线程执行程序中完全运行，我们就可以删除委托
		 */
		// unfortunately, this method does not run in the scheduler executor, but in the
		// checkpoint coordinator time thread.
		// we can remove the delegation once the checkpoint coordinator runs fully in the scheduler's main thread executor
		/*************************************************
		 * 
		 *  注释： Operator 收到 checkpoint coordinator 的通知, 调用内部实现执行处理
		 */
	...
	public void checkpointComplete(long checkpointId) {
		...
		// TODO_MA 注释： checkpoint 成功， OperatorCoordinatorHolder 的后续处理
	...
	private void checkpointCompleteInternal(long checkpointId) {
		...
		/*************************************************
		 * 
		 *  注释： 告知 checkpoint coordinator 成功了
		 */
	...
	public void afterSourceBarrierInjection(long checkpointId) {
		...
		// fortunately, the event-sending goes pretty much directly to the RPC gateways, which are thread safe.
	...
	public void abortCurrentTriggering() {
		 ...
		 *  注释： 取消
		 */
		// this will automatically be fixed once the checkpoint coordinator runs in the scheduler's main thread executor
		eventValve.openValveAndUnmarkCheckpoint();
	}
