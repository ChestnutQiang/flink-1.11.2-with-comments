...
public abstract class SchedulerBase implements SchedulerNG {
	...
	private void tryRestoreExecutionGraphFromSavepoint(ExecutionGraph executionGraphToRestore,
		...
		if(savepointRestoreSettings.restoreSavepoint()) {
			...
			if(checkpointCoordinator != null) {
				...
				/*************************************************
				 *
				 *  注释： 进入 checkpoint state restore 流程
				 */
	...
	protected void transitionToScheduled(final List<ExecutionVertexID> verticesToDeploy) {
		...
		// TODO_MA 注释： 将每个 ExecutionVertex 变成 调度 状态
		...
		if(checkpointCoordinator != null) {
			ioExecutor.execute(() -> {
				try {
					...
					/*************************************************
					 *
					 *  注释： JobMaster 通知 checkpointCoordinator
					 */
	...
	public void declineCheckpoint(final DeclineCheckpoint decline) {
		...
		// TODO_MA 注释： 拿到 CheckpointCoordinator
		final CheckpointCoordinator checkpointCoordinator = executionGraph.getCheckpointCoordinator();

		// TODO_MA 注释： 拿到发送取消 checkpint 的 TaskManagerLocation 位置信息
	...
	private String retrieveTaskManagerLocation(ExecutionAttemptID executionAttemptID) {
		...
		/*************************************************
		 *
		 *  注释： 根据当前 Task被分配的 TaskSlot 来获取 TaskManager 所在地信息
		 */
