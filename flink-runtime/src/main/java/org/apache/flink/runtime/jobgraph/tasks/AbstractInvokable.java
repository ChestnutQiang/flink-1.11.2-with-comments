...
/**
 * // TODO_MA 注释： 这是 TaskManager 可以执行的每个 Task 的抽象基类。 具体任务扩展了此类，例如流任务和批处理任务。
 ...
 *
 * // TODO_MA 注释： 执行任务时，TaskManager会调用{@link #invoke（）}方法。
 * // TODO_MA 注释： 任务的所有操作都以这种方法发生（设置输入输出流读取器和写入器以及任务的核心操作）
 * <p>The TaskManager invokes the {@link #invoke()} method when executing a task.
 * All operations of the task happen in this method (setting up input
 * output stream readers and writers as well as the task's core operation).
 *
 * // TODO_MA 注释： 所有扩展的类都必须提供一个构造函数{@code MyTask（Environment，TaskStateSnapshot）}。
 * // TODO_MA 注释： 为了方便起见，总是无状态的任务也只能实现构造函数* {@code MyTask（Environment）}
 ...
 *
 * // TODO_MA 注释： 开发人员说明：尽管构造函数无法在编译时强制执行，但我们尚未冒险引入工厂
 * // TODO_MA 注释： （毕竟它只是内部API，对于Java 8，*可以使用{@code Class :: new }几乎就像工厂的lambda。
 ...
 *
 * // TODO_MA 注释： 没有接受初始任务状态快照并将其存储在变量中的构造函数。这是有目的的，
 * // TODO_MA 注释： 因为AbstractInvokable本身不需要状态快照（仅子类（例如StreamTask才需要状态））
 * // TODO_MA 注释： 并且我们不想无限期地存储引用，从而防止垃圾收集器清除初始状态结构。
 ...
 *
 * // TODO_MA 注释： 任何支持可恢复状态并参与*检查点的子类都需要重写
 * // TODO_MA 注释： {@link #triggerCheckpointAsync（CheckpointMetaData，CheckpointOptions，boolean）}，
 * // TODO_MA 注释： {@link #triggerCheckpointOnBarrier（CheckpointMetaData，CheckpointOptions
 * ，CheckpointMetrics）}，* {@link #abortCheckpointOnBarrier（ long，Throwable）}和{@link #notifyCheckpointCompleteAsync（long）}。
...
public abstract class AbstractInvokable {
	 ...
	 * is called by the task manager when the actual execution of the task starts.
	 *
	...
	public <E extends Exception> void executeInTaskThread(ThrowingRunnable<E> runnable, String descriptionFormat, Object... descriptionArgs) throws E {
		throw new UnsupportedOperationException(String.format("executeInTaskThread not supported by %s", getClass().getName()));
	}
