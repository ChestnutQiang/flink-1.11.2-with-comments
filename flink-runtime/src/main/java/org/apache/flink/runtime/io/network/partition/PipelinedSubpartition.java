...
public class PipelinedSubpartition extends ResultSubpartition {
	...
	// TODO_MA 注释： 当前 subpartiion 堆积的所有的 Buffer 的队列
	/**
	 * All buffers of this subpartition. Access to the buffers is synchronized on this object.
	 */
	private final ArrayDeque<BufferConsumer> buffers = new ArrayDeque<>();

	// TODO_MA 注释： 当前 subpartiion 中堆积的 buffer 的数量
	/**
	...
	// TODO_MA 注释： index 是当前 sub-paritition 的索引
	...
	public boolean add(BufferConsumer bufferConsumer, boolean isPriorityEvent) throws IOException {
		...
		// TODO_MA 注释： 添加一个新的BufferConsumer
	...
	private boolean add(BufferConsumer bufferConsumer, boolean finish, boolean insertAsHead) {
		...
		synchronized(buffers) {
			...
			// TODO_MA 注释： 更新 backlog 的数量，只有 buffer 才会使得 buffersInBacklog + 1，事件不会增加 buffersInBacklog
	...
	private void handleAddingBarrier(BufferConsumer bufferConsumer, boolean insertAsHead) {
		...
		if(insertAsHead) {
			checkState(inflightBufferSnapshot.isEmpty(), "Supporting only one concurrent checkpoint in unaligned " + "checkpoints");
	...
	BufferAndBacklog pollBuffer() {
		synchronized(buffers) {
			...
			// will be 2 or more.
			return new BufferAndBacklog(buffer, isDataAvailableUnsafe(), getBuffersInBacklog(), isEventAvailableUnsafe());
		}
	}
	...
	public PipelinedSubpartitionView createReadView(BufferAvailabilityListener availabilityListener) throws IOException {
		...
		synchronized(buffers) {
			...
			LOG.debug("{}: Creating read view for subpartition {} of partition {}.", parent.getOwningTaskName(), getSubPartitionIndex(),
				parent.getPartitionId());

			/*************************************************
			 *
			 *  注释： 创建一个 PipelinedSubpartitionView
			 */
	...
	public String toString() {
		...
		return String
			.format("PipelinedSubpartition#%d [number of buffers: %d (%d bytes), number of buffers in backlog: %d, finished? %s, read view? %s]",
	...
	private boolean shouldNotifyDataAvailable() {
		...
		// TODO_MA 注释： 只在第一个 buffer 为 finish 的时候才通知
	...
	private void notifyDataAvailable() {
		...
		if(readView != null) {
			...
			// TODO_MA 注释： 这个 ResultSubPartition 对应一个消费者： PipelinedSubpartitionView
