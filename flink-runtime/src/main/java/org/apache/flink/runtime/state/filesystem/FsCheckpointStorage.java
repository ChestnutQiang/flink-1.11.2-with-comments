...
public class FsCheckpointStorage extends AbstractFsCheckpointStorage {
	...
	public FsCheckpointStorage(Path checkpointBaseDirectory, @Nullable Path defaultSavepointDirectory, JobID jobId, int fileSizeThreshold,
	...
	public FsCheckpointStorage(FileSystem fs, Path checkpointBaseDirectory, @Nullable Path defaultSavepointDirectory, JobID jobId,
		...
		/*************************************************
		 * 
		 *  注释： 拿到 checkpoint directory
		 *  1、checkpointBaseDirectory 父目录
		 *  2、jobId 子目录
		 */
	...
	public CheckpointStreamFactory resolveCheckpointStorageLocation(long checkpointId,
		...
		if(reference.isDefaultReference()) {
			...
			/*************************************************
			 * 
			 *  注释： 封装得到 FsCheckpointStorageLocation 返回
			 */
			return new FsCheckpointStorageLocation(fileSystem, checkpointDir, sharedStateDirectory, taskOwnedStateDirectory, reference,
				fileSizeThreshold, writeBufferSize);
		} else {
			// location encoded in the reference
			final Path path = decodePathFromReference(reference);

			return new FsCheckpointStorageLocation(path.getFileSystem(), path, path, path, reference, fileSizeThreshold, writeBufferSize);
		}
	}
